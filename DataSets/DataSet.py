...
class DataBase(): 
    def __init__(self, f, size):
        self._size = size
        self._dim = f._dim
        self._X = torch.zeros(size, f._dim);
    ...
    def create(self, seed = torch.rand(1)):
        ...
        self._X = torch.rand(self._size, self._obj._dim, dtype=float)
        self._y = torch.from_numpy(self._obj.evaluate(self._X.numpy())).unsqueeze(-1)
    ...
    def par_create(self, comm, seed = torch.rand(1)):
        ...
        if my_rank == 0:
            torch.manual_seed(seed)
            candidates = torch.rand(self._size, self._obj._dim, dtype=float)
            self._X = None
            ...
            for c in range(int(n_cand[0])):
                y_new = self._obj.evaluate(candidates[n_proc*c].numpy())
                if self._X == None :
...
#            print('Try distance in creation')
            ...
            y_new = []
            for c in range(n_cand[my_rank]):
                y_new.append(self._obj.evaluate(cand[c]))
            ## Send it back
            for c in range(n_cand[my_rank]):
...
#            print('Adding random perturbation of minimum candidate : \n', new_cand)
    ...
    def print_min(self):
        ...
        print('Minimum found value is ', self._y[i_min], ' realized at point ', self._obj.unmap(self._X[i_min]))
